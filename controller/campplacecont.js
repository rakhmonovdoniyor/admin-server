...
const CampPlace = require("../models/campPlacerout");
...
const AddNewMotor = async (req, res) => {
    try {
        ...
        const newMotor = new CampPlace({name, brand, cost, company, license, location, people, type, date });
        await newMotor.save();
...
const GetAllMotorData = async (req, res) => {
    try {
        const motors = await CampPlace.find();
...
const EditMotor = async (req, res) => {
    try {
        ...
        const motor = await CampPlace.findOne({name});
...
const DeleteMotor = async (req, res) => {
    try {
        const { name } = req.params;
        const motor = await CampPlace.findOneAndDelete({name});

        if (motor) {
            console.log(`Deleted Data: ${name}`);
            res.status(200).json({ message: `Deleted Data`, motor });
        } else {
            res.status(404).json({ message: "CampPlace not found" });
        }
