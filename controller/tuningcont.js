...
const Tuning = require("../models/tuning-rout");
...
const AddNewMotor = async (req, res) => {
    try {
        ...
        const newMotor = new Tuning({name, brand, cost, company, license, location, people, type, date });
        await newMotor.save();
...
const GetAllMotorData = async (req, res) => {
    try {
        const motors = await Tuning.find();
...
const EditMotor = async (req, res) => {
    try {
        ...
        const motor = await Tuning.findOne({name});
...
const DeleteMotor = async (req, res) => {
    try {
        const { name } = req.params;
        const motor = await Tuning.findOneAndDelete({name});

        if (motor) {
            console.log(`Deleted Data: ${name}`);
            res.status(200).json({ message: `Deleted Data`, motor });
        } else {
            res.status(404).json({ message: "Tuning not found" });
        }
